...
class ArrayDnmc {
	...
	friend ostream& operator<<(ostream&, const elm& e);
private:
	elm* arr;
	int size;//从1开始
	int capacity;//从1开始
public:
	...
	ArrayDnmc(const ArrayDnmc& arrdt) :size(arrdt.size), capacity(arrdt.capacity) {//拷贝构造(深拷贝)
...
template<typename elm>
ostream& operator<<(ostream& out, const elm& e) {
	for (int i = 0; i < size; i++)
		out << e[i] << " ";
	out << endl;
	return out;
